...
# 延迟加载 ReasoningEngine（兼容多种部署路径）
_reasoning_engine_cls = None

def _get_reasoning_engine_cls():
    global _reasoning_engine_cls
    if _reasoning_engine_cls is None:
        try:
            from reasoning_engine import ReasoningEngine
            _reasoning_engine_cls = ReasoningEngine
        except ImportError:
            try:
                from src.reasoning_engine import ReasoningEngine
                _reasoning_engine_cls = ReasoningEngine
            except ImportError:
                logger.warning("无法加载 reasoning_engine 模块，使用空引擎")
                _reasoning_engine_cls = None
...
class StockAnalysisReport:
    ...
    def __init__(self, symbol: str, latest_signal: dict = None, financial_data: dict = None, warnings: list = None):
        ...
        # Phase 2 新增：文字解读（延迟导入）
        cls = _get_reasoning_engine_cls()
        if cls:
            self.reasoning_engine = cls()
        else:
